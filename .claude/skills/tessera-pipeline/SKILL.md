...
Invoke the **tessera-lab-parse** workflow on the user's PDF. Read `.claude/skills/tessera-lab-parse/SKILL.md` for the parse contract. Output: `biomarkers.json` + `panel-completeness.json`.
...
Invoke the **tessera-intake-v2** workflow. Read `.claude/skills/tessera-intake-v2/SKILL.md`. The intake-v2 has 5 modules (base + HPI + family-depth + constraints + environment + mental-health-screen). If `intake.md` is provided, parse it; if not, ask each module's questions inline.
...
Invoke **tessera-rule-evaluate**. Read `.claude/skills/tessera-rule-evaluate/SKILL.md`. This is a deterministic pass — it does not author prose. Output: `rule-evaluation.json` listing every fired rule, its tier, its triggering markers, and the rulebook hash.

### Step 6 — Classify phenotype + root causes

Invoke **tessera-rootcause-phenotype**. Read `.claude/skills/tessera-rootcause-phenotype/SKILL.md`. Output: `phenotype.json` + `rootcause.json`. The phenotype label drives report voice; the root-cause graph names 2–3 mechanisms to attack.

### Step 7 — Safety screen

Invoke **tessera-safety-screen**. Read `.claude/skills/tessera-safety-screen/SKILL.md`. Cross-checks every supplement / intervention from the rule-evaluation against the safety matrix using intake's medications / conditions / allergens / pregnancy. Output: `safety.json` with blockers, warnings, cleared.

### Step 8 — Author the 5-domain protocol

Invoke **tessera-protocol-author**. Read `.claude/skills/tessera-protocol-author/SKILL.md`. Consumes everything above and produces `protocol.json` — the 5 domain cards (Nutrition, Training, Recovery, Supplements, Mind), the 90-day plan, and the marker-by-marker retest cadence.

### Step 9 — Compose the report

Invoke **tessera-report-compose**. Read `.claude/skills/tessera-report-compose/SKILL.md`. Composes the populated HTML using the Tessera design system, then renders to PDF via headless Chrome. Output: `report.html` + `report.pdf`.
...
```
.claude/skills/tessera-pipeline/
└── SKILL.md          ← you are here (orchestrator)

.claude/skills/tessera-lab-parse/       ← step 2
.claude/skills/tessera-intake-v2/       ← step 3
.claude/skills/tessera-wearable-ingest/ ← step 4 (stub)
.claude/skills/tessera-genetics-ingest/ ← step 4 (stub)
.claude/skills/tessera-rule-evaluate/   ← step 5
.claude/skills/tessera-rootcause-phenotype/ ← step 6
.claude/skills/tessera-safety-screen/   ← step 7
.claude/skills/tessera-protocol-author/ ← step 8
.claude/skills/tessera-report-compose/  ← step 9

.claude/skills/lib/        ← shared knowledge (rulebook, ranges, mechanisms, phenotypes, safety matrix)
.claude/skills/tessera-report/  ← legacy alias, routes here
```
